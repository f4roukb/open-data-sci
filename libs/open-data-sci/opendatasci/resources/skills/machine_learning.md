...
- The budget itself should be sized to the machine executing the search: an extensive search that is routine on a many-core or GPU-equipped machine can take days on a modest one — check the available hardware resources (cores, free RAM, accelerators) before committing, and scale trial counts, per-trial cost, and parallelism to what the machine can actually sustain
