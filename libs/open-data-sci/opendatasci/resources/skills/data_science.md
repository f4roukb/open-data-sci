...
- Computational effort should match the machine doing the work: check the available hardware resources before launching anything expensive (wide cross-validation, large searches, big in-memory joins) — a plan that is routine on a powerful machine can run for hours on a weak one, and a subsample, a simpler model, or fewer folds often answers the question just as well
