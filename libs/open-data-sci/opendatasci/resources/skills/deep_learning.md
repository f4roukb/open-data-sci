...
- The available hardware can realistically carry the training plan — model size, batch size, and precision are bounded by RAM/VRAM, and an architecture sized for a GPU can take days on CPU; check the machine's hardware resources before designing the model and scale depth, width, batch size, and epochs to what it can sustain (or prefer a classical model when the hardware is modest)
