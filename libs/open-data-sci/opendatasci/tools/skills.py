...
def create_skill_tools(store: BaseSkillStore) -> list[BaseTool]:
    ...
    return [_create_load_skill_tool(store), _create_list_skills_tool(store)]


def _create_load_skill_tool(store: BaseSkillStore) -> BaseTool:
    @tool
    def load_skill(
        summary: str,
        communication: str,
        ...
        if skill_domain_name is not None:
            ...
            if current_skill_domain is not None and current_skill_domain.name == skill_domain_name:
                ...
                skill_domain: SkillDomain | None = store.load_domain(skill_domain_name)
                if skill_domain is None:
                    available = ", ".join(sorted(store.list_domains())) or "(none)"
                    return Command(
                        update={
                            "messages": [
                                ToolMessage(
        ...
        if skill_name is not None:
            ...
            if current_skills and current_skills[0].name == skill_name:
                ...
                loaded: Skill | None = store.load(skill_name)
                if loaded is None:
                    available = ", ".join(sorted(store.list_skills())) or "(none)"
                    return Command(
                        update={
                            "messages": [
                                ToolMessage(
...
def _create_list_skills_tool(store: BaseSkillStore) -> BaseTool:
    @tool
    def list_skills() -> str:
        ...
        """
        domains = sorted(store.list_domains())
        standalone_skills = sorted(name for name in store.list_skills() if "::" not in name)
