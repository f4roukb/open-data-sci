...
class TestListWorkspaceFiles:
    @pytest.mark.asyncio
    async def test_no_workspace_path_returns_no_active_workspace(self) -> None:
        tool = _get_tool(None)
        result = await tool.ainvoke({"summary": "Listing files", "communication": "listing"})
        assert "No active workspace" in result

    @pytest.mark.asyncio
    async def test_empty_directory_returns_is_empty_message(self, tmp_path: Path) -> None:
        tool = _get_tool(tmp_path)
        result = await tool.ainvoke({"summary": "Listing files", "communication": "listing"})
        assert "empty" in result.lower()

    @pytest.mark.asyncio
    async def test_lists_files_with_absolute_paths(self, tmp_path: Path) -> None:
        f = tmp_path / "data.csv"
    ...
    @pytest.mark.asyncio
    async def test_does_not_list_files_with_relative_paths(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_includes_workspace_name_in_output(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_shows_nested_file_with_absolute_path(self, tmp_path: Path) -> None:
        subdir = tmp_path / "subdir"
        subdir.mkdir()
    ...
    @pytest.mark.asyncio
    async def test_excludes_hidden_directories(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_shows_file_size_in_bytes_for_small_file(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_shows_file_size_in_kb_for_medium_file(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_shows_file_size_in_mb_for_large_file(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_multiple_files_all_listed_with_absolute_paths(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_nested_files_listed_with_absolute_paths(self, tmp_path: Path) -> None:
