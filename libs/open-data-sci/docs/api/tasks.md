...
- **`BackgroundTaskRecord`** — a point-in-time snapshot of one background task: its `summary`, `status`, timestamps, its plain-text `activity` log (one entry per tool call the worker makes — see [Activity log](#activity-log) below), and its `result` or `error` once it reaches a terminal state.
- **`BackgroundTaskStatus`** — `running`, `completed`, `failed`, or `cancelled`.
- **`BackgroundTaskUpdate`** / **`BackgroundTaskUpdateKind`** — one event pushed against a task: `completed` (the task reached a terminal state — `status`/`result`/`error` are populated) or `progress` (a monitor's regex matched the task's activity — `monitor_id`/`pattern`/`matched_texts` are populated; see [Monitoring task activity](#monitoring-task-activity)). `BackgroundTaskUpdate.to_message()` renders either kind as the `TaskMessage` fed back to the model.
...
Every background task accumulates a plain-text `activity` log on its `BackgroundTaskRecord`: one entry per tool call the worker makes (`<tool>name</tool>\n<output>output</output>`), appended as each call completes. It's capped at 200 entries and 32KB per entry (oldest entries/characters dropped first) to bound memory on a long-running worker, and it's readable through the `check_task` tool without any extra setup.
...
Matching happens against the same 32KB-truncated copy that gets persisted to `activity`, not the raw entry — this bounds regex execution time to a fixed-size string regardless of how much a single tool call actually printed, at the cost of never matching text past the truncation cutoff. Only the newly-appended entry is scanned each time, never a task's prior activity, so a monitor is never re-triggered by text it has already seen. Every match found in the triggering entry (not just the first) is carried in that one update, numbered `Match 0: ...`, `Match 1: ...`, and so on.
