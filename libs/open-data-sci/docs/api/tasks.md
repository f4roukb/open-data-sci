...
Alongside `watch_completions()`, the manager also exposes a second, independent way to retrieve completions: `drain_context_updates()` returns and clears every completed record collected since the last drain, and `has_context_updates()` is a cheap, non-blocking peek at whether a drain would return anything. Where `watch_completions()` is a notification stream (for a caller that wants to know *that* something finished, e.g. to show a UI message), the drain methods are a content buffer (for a caller that wants the record itself, on its own schedule). The two are independent — draining one does not consume the other, so each can have its own consumer without racing.
