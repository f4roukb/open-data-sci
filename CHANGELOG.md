...
- **Background (non-blocking) worker scheduling** — the `task` tool (see rename below) gains a `synch_mode` argument (`SynchMode.SYNC`, the default, or `SynchMode.ASYNC`): `"async"` schedules the subtasks in the background and returns a task ID immediately instead of blocking on completion. New `get_task_status` (poll by task ID, or list every tracked task) and `cancel_task` (best-effort cancellation) main-agent-only tools back this. New `opendatasci.tasks` package (`BaseTaskManager` interface, `LocalTaskManager` in-process `asyncio.Task`-backed implementation, `TaskRecord`, `TaskStatus`) tracks scheduled work for the lifetime of the session.
...
- **`spawn_workers` tool renamed to `task`** (`ToolName.SPAWN_WORKERS` → `ToolName.TASK`), reflecting the new background-scheduling and parallel-execution options above; `ToolName` also gains `GET_TASK_STATUS` and `CANCEL_TASK`. **Breaking:** any code referencing the tool by its old name (`"spawn_workers"`, `ToolName.SPAWN_WORKERS`) needs updating.
- **`WorkerTask` is now `SpawnWorkersTool.TaskDetails`**, nested under the tool class instead of module-level; `opendatasci.tools` now exports `SpawnWorkersTool` in place of `WorkerTask`. **Breaking:** `from opendatasci.tools import WorkerTask` no longer works.
- **`create_worker_tools()` requires a new `task_manager: BaseTaskManager` argument** (shared with `create_task_management_tools()` so `get_task_status`/`cancel_task` can see tasks scheduled by `task`). **Breaking:** existing callers must pass a `task_manager` (e.g. `LocalTaskManager()`).
