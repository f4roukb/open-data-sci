...
- **Background (non-blocking) worker scheduling** — the `task` tool (see rename below) gains a `run_mode` argument (`RunMode.FOREGROUND`, the default, or `RunMode.BACKGROUND`): background mode submits each subtask as its own tracked task (one `task_id` per subtask, not one per `task` call) and returns immediately instead of blocking on completion. New main-agent-only tools back this: `check_task` (single task's status, timestamps, progress, and result/error, as JSON) and `list_tasks` (a table of tasks filtered by status, `{RUNNING}` by default); `cancel_task` (best-effort cancellation) is unchanged. New `opendatasci.tasks` package (`AgentTaskManagerBase` interface, `LocalAgentTaskManager` in-process `asyncio`-backed implementation, `AgentTaskRecord`, `AgentTaskStatus`, `AgentTaskProgressReport`/`AgentTaskProgressUpdate`) tracks scheduled work for the lifetime of the session. `AgentTaskRecord.progress`/`AgentTaskProgressReport`/`AgentTaskProgressUpdate` are part of the data model and surfaced through `check_task`, but nothing currently populates them — worker-side progress reporting is being redesigned.
...
- **`tools/workers.py` merged into `tools/tasks.py`** — task creation (`task`) and task management (`check_task`/`list_tasks`/`cancel_task`) are all task tools, so they now live together. `tools/factory.py`'s `create_worker_tools()` is renamed `create_task_tools()` and returns only the `task` tool; the former `create_task_tools()` (management tools) is renamed `create_task_management_tools()`. Both take the same shared `agent_task_manager: AgentTaskManagerBase` so `check_task`/`list_tasks`/`cancel_task` can see tasks scheduled by `task`. **Breaking:** existing callers of `create_worker_tools()`/`create_task_tools()` need updating to the new names.
- **`AgentTaskManagerBase`/`LocalAgentTaskManager` methods renamed for clarity**: `submit` → `submit_task`, `list` → `list_tasks`, `cancel` → `cancel_task`. **Breaking:** existing callers need updating to the new names.
- **`AgentTaskManagerBase` never had a `report_progress` and never will** — a task manager exposes reading tasks (`get_task_record`/`list_tasks`), not writing to them; `submit_task`'s `work` callable only receives the `task_id`, not the record. Publishing progress is a concern of whatever runs the work.
...
- **`create_task_tools()`'s `skill_store` argument is now mandatory** — it silently defaulted to a fresh `LocalSkillStore(...)` rooted at `context.root` when omitted, which could construct a store disconnected from the one every other tool in the same tool set was using; callers must now pass the store explicitly, and the now-unused `context` parameter is dropped.
- **Store-typed parameters and fields renamed to the `*_store` convention** (e.g. `context` → `context_store`, `store` → `skill_store`) across `tools/factory.py`, `tools/modes.py`, `tools/skills.py`, `tools/tasks.py`, and `tools/dataset_info.py`, for consistency with `Agent.__init__`'s existing `context_store`/`skill_store` naming. Along the way, fixed a latent bug in `create_execution_mode_tools`'s call into `create_task_tools()`, which passed a `store=` keyword that didn't match either the old (`skill_store`) or new parameter name and would have raised `TypeError` the moment a caller stopped relying on the default. **Breaking:** any code constructing these tools/factories with the old keyword argument names (`context=`, `store=`) needs updating.
